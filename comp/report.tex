...
\item \textbf{承重墙 1 · Attack Genome}：首次把 37 维基因空间
      正式定义为\textbf{感知攻击面}，配合 Gene-Oriented 威胁模型
      （敌手—能力—收益三件套），把"鲁棒性研究"升级为"安全审计"。
\item \textbf{承重墙 2 · Cross-Architecture Failure Law}：
      6/6 跨规划器 XGBoost AUC $> 0.70$（最强 CNN$\to$DINO 达 $0.798$），
      $\{s^*, w\}$ 跨架构方差 $0.013$，
      即"失败的\textbf{几何}在表征间守恒"——
      区别于"失败的幅度（fail rate）"的传统报告。
\item \textbf{承重墙 3 · Failure Basin 数学定义}：
      把"Basin"从经验现象提升为可定义、可测量、可反驳的几何对象：
      \begin{equation*}
        \mathcal{B}_{\tau} = \{ \mathbf{g} \in \mathcal{G} \mid P(\text{success} \mid \mathbf{g}) < \tau \},
        \quad
        s^*,\; w,\; M
      \end{equation*}
      4 个核心算子全部由 $88\,560$ 真实 planner 决策
      独立验证（不依赖 XGBoost），89.2\% 单调率 + CNN basin
      1.5$\times$ 宽于 DINO/TF。
\item \textbf{可操控}（因果验证）：per-sample SHAP top-$K$ pushback
      触发 86\% / 71\% / 100\% flip (CNN/DINO/TF, $K\!=\!10$)，
      证明基因组合可被反向推动修复；
      同因异果：$88\%$ / $82\%$ / $80\%$ 的失败样本中
      \gene{edge\_mean} 是 per-sample top1 驱动基因，
      印证"跨表征不变基因 $=$ 最强因果杠杆 $=$ 跨架构通用武器"。
\end{enumerate}

\textbf{创新性}：（1）\textit{首次}把感知基因正式定义为\textbf{攻击面}；
（2）\textit{首次}发现 Failure Basin 几何在跨视觉表征上守恒
（含数学定义 $\mathcal{B}_{\tau}, s^*, w, M$）；
（3）\textit{首次}给出可被 reviewer 重复的反事实验证协议
（per-sample SHAP pushback + 独立验证层 L5），
将"预测有效"升级为"因果可操控"；
（4）\textit{首次}给出 Conserved vs Architecture-Specific Gene
二分（$\sigma$ 分析），揭示攻击者的最优化攻击图谱。

\textbf{实用性}：本框架对防御侧（指出 DINO/TF 升级仅衰减纹理类攻击而
\textbf{不能}消除对结构信息的依赖）和攻击侧（联合扰动 top-$K$ SHAP
基因即可跨表征通用）均给出可操作的工程指引。
...
本作品的测试分为四个层级，对应 [[attack-genome-law-hunting]]
中"Observation $\to$ Pattern $\to$ Law $\to$ Causality"
的递进式证据链。

\begin{description}[leftmargin=1.4em,style=nextline]
\item[Observation] 测三类规划器在 492 scene $\times$ 10 attack
      $\times$ 6 strength 上的失败率，验证表征差异。
\item[Pattern] 测同规划器 gene$\to$fail XGBoost AUC，
      验证基因空间对失败的预测能力。
\item[Law (跨表征可预测)] 测 6 对跨规划器 XGBoost 迁移 AUC，
      验证失败形状在表征间的守恒性。
\item[Causality (反事实)] 用 per-sample SHAP pushback 测 flip rate，
      验证因果可操控性，并以 random-K baseline 排除平凡信号。
\end{description}
...
\noindent\textbf{剂量响应（dose-response）单调上升}：
CNN Rain 上 K=1 12.5\% $\to$ K=5 50.0\% $\to$ K=10 86\%（XGBoost 已知）——
这是\textbf{基因干预越彻底 $\Rightarrow$ planner 行为越被修复}的定量证据。

\noindent\textbf{⭐ 跨架构核心非对称（L3 揭示）}：
同样的 image-level gene 干预，CNN 修复 50\%，DINO 修复 0\%。DINO
0\% \textbf{不是方法失败}，而是 \textbf{DINO 的 OOD 鲁棒性设计
在 gene 层面也成立}——它的自监督特征天然抗 gene-level 扰动。
这揭示了：
\begin{itemize}
...
\noindent\textbf{核心定位}：本作品\textbf{唯一的核心贡献}是
"Failure Basin 几何在跨视觉表征上的守恒性"。
本节将 3 个支撑性贡献 + 1 个 case study 按\textbf{支撑强度}排序
（由"承重墙"到"演示"）：
...
在 88\,560 样本的规模上，我们从 Observation $\to$ Pattern $\to$ Law
$\to$ Causality 四级证据链证实：
...
\begin{itemize}
\item \textbf{对防御方}：提示数据增强应直接 augment edge / lane / 道路结构，
      而不是依赖 backbone 升级。
\item \textbf{对攻击方}：联合扰动 SHAP top-$K$ 基因可作为
      跨规划器通用 attack recipe。
