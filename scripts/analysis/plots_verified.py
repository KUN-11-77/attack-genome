"""Tier C++ plots — using verified parse logic."""
import sys
import pandas as pd
import numpy as np
import matplotlib
...
# Parse 3 shards (verified logic from debug2)
shards = []
for s in [0, 1, 2]:
    p = "/data3/khsong/cogatedrive/outputs/ag_tier_cpp_shard%s/summary.txt" % s
    with open(p) as f:
        text = f.read()
...
print("shards:", len(shards), file=sys.stderr)
assert len(shards) > 0, "no rows: " + str(len(shards))
...
for i, atk in enumerate(attacks):
    ...
    for p in planners:
        ...
        if len(sub) == 0:
            continue
        ax.plot(strengths, sub.iloc[0]["asrs_list"], "-o",
                color=colors[p], label=p, lw=2, ms=6)
...
# Fig 2
imp = pd.read_csv(
    "/data3/khsong/cogatedrive/exp/failure_factor/feature_importance.csv",
    index_col=0)
...
plt.tight_layout()
plt.savefig(OUT / "fig2_gene_importance.png", dpi=150, bbox_inches="tight")
plt.close()
print("fig2 done", file=sys.stderr)

# Fig 3
csv = pd.read_csv(
    "/data3/khsong/cogatedrive/outputs/ag_tier_cpp_merged/per_sample_genes.csv")
...
for f in sorted(os.listdir(OUT)):
    print("  " + f + ": " + str((OUT / f).stat().st_size // 1024) + " KB",
          file=sys.stderr)
print("ALL 3 PLOTS DONE", file=sys.stderr)
