"""Tier C++ plot script — all 3 figures in one run."""
import os, sys
...
print("parsed %d rows" % len(shards))
assert len(shards) > 0, "no rows parsed!"
...
for i, atk in enumerate(attacks_order):
    ...
    for p in planners:
        ...
        ax.plot(strengths, asrs, "-o", color=colors[p], label=p, lw=2, ms=6)
...
print("=== ALL 3 figs saved to", OUT)
for f in sorted(os.listdir(OUT)):
    print("  %s: %d KB" % (f, (OUT / f).stat().st_size // 1024))
